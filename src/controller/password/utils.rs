use std::{error::Error, io::{ErrorKind, self}, iter::repeat};

use crypto::{aes_gcm::AesGcm, aead::{AeadDecryptor, AeadEncryptor}};
...
fn get_valid_key(key: &str) -> Vec<u8> {
    ...
    if length < 16 {
        ...
        for i in 16..old_bytes.len() {
            bytes[i%16] += old_bytes[i];
        }
    }
    bytes
}
